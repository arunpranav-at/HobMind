  ...
  return (
    <motion.div layout whileHover={{ y: -6 }} className="tech-card">
      <div className="flex items-start justify-between">
        <div>
          <h3 className={`tech-title text-lg sm:text-xl ${status === 'dropped' ? 'line-through text-red-400' : ''}`}>{technique.title}</h3>
          <p className={`tech-desc text-sm sm:text-base ${status === 'dropped' ? 'line-through text-red-400' : ''}`}>{technique.description}</p>
        </div>
      ...
      <div className="mt-4 flex gap-3 flex-wrap">
          ...
          {showStatusButtons && (
            <>
              <button
                className={`px-3 sm:px-4 py-2 rounded-lg ${status === 'mastered' ? 'ring-2 ring-green-400' : ''} ${status === 'mastered' ? 'opacity-60 cursor-not-allowed' : ''}`}
              ...
              <button
                className={`px-3 sm:px-4 py-2 rounded-lg ${status === 'in-progress' ? 'ring-2 ring-yellow-400' : ''} ${status === 'in-progress' ? 'opacity-60 cursor-not-allowed' : ''}`}
              ...
              <button
                className={`px-3 sm:px-4 py-2 rounded-lg ${status === 'dropped' ? 'ring-2 ring-red-400' : ''} ${status === 'dropped' ? 'opacity-60 cursor-not-allowed' : ''}`}
